...
dt.unemployment[,'2019':=dt.unemploymentData2$`2019`[match(Area,dt.unemploymentData2$Name)]]#add data to 2019 column from dt.unemploymentData2
...
colnames(dt.poverty) = as.character(dt.poverty[1,]) #set first row to header of table 
...
ggplot(dt.unemployment,aes(x=year,y=unemployment_percent,color=factor(party),group=factor(Area)))+geom_line(size=1) + scale_colour_manual(values = c("dodgerblue3", "black", "firebrick1"))+facet_wrap(~Area) 
...
ggplot(dt.povertyAvgDelta,aes(x=year,y=avg_delta_party,color=factor(party),group=factor(party)))+geom_line(size=2) + scale_colour_manual(values = c("dodgerblue3", "firebrick1"))
...
ggplot(data = dt.crime, aes(x=as.character(year), y=delta)) + geom_boxplot(aes(fill=party)) + scale_fill_manual(values=c("dodgerblue3","lightgrey","firebrick1")) 
...
ggplot(dt.crimeAvgDelta,aes(x=year,y=avg_delta_party,color=factor(party),group=factor(party)))+geom_line(size=2) + scale_colour_manual(values = c("dodgerblue3", "firebrick1"))
...
ggplot(dt.unemploymentAvgDelta,aes(x=year,y=avg_delta_party,color=factor(party),group=factor(party)))+geom_line(size=2) + scale_colour_manual(values = c("dodgerblue3", "firebrick1"))
